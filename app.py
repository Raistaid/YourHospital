...
def retdocsandapps(docid):
    ...
    conn.commit()
    docsandapps = c.fetchall()
    docsandapps2 = [i for i in docsandapps if str(i[0]) == str(docid)]
...
def retapprequests(docid):
    ...
    conn.commit()
    appreq = c.fetchall()
    appreq2 = [i for i in appreq if str(i[0]) == str(docid)]
    logger.debug(f"Найдено {len(appreq2)} запросов для врача")
    return appreq, len(appreq2)
...
def ret_doctor_reg_requests():
    ...
    conn.commit()
    data = c.fetchall()
    doctor_reg_requests = [d for d in data if str(d[-1]) == '0']
...
def ret_registered_patients():
    ...
    conn.commit()
    data = c.fetchall()
    registered_patients = [d for d in data if str(d[-1]) == '1']
...
def ret_registered_doctors():
    ...
    conn.commit()
    data = c.fetchall()
    registered_doctors = [d for d in data if str(d[-1]) == '1']
...
def ret_docname_docspec():
    ...
    docname_docid = [f"{doc[0]} {doc[1]} | {doc[2]} | {doc[3]}" for doc in registered_doctors]
...
def get_all_docids():
    ...
    conn.commit()
    registered_doctors = c.fetchall()
    docids = [str(i[5]) for i in registered_doctors]
...
def get_all_patnums():
    ...
    patnums = [str(i[3]) for i in registered_patients]
...
def deletepatient():
    ...
    return render_template('adminlogin.html', patient_reg_requests=patient_reg_requests,
                           doctor_reg_requests=doctor_reg_requests, registered_patients=registered_patients,
                           registered_doctors=registered_doctors, l1=len(patient_reg_requests), l2=len(doctor_reg_requests), l3=len(registered_patients), l4=len(registered_doctors))
...
def makeappointment():
    try:
        ...
        c.execute('''
            ...
            VALUES (?, ?, ?, ?)''', (docid, patname, phn, appdate))
        conn.commit()
...
def approvedoctor():
    ...
    try:
        ...
        for i in doctor_requests:
            ...
            logger.warning("Врач для одобрения не найден")
    except Exception as e:
        logger.exception("Ошибка при одобрении врача")

    patient_reg_requests = ret_patient_reg_requests()
    doctor_reg_requests = ret_doctor_reg_requests()
    registered_patients = ret_registered_patients()
    registered_doctors = ret_registered_doctors()
    return render_template('adminlogin.html', mess='Врач успешно одобрен',
                           ...
                           l1=len(patient_reg_requests), l2=len(doctor_reg_requests), l3=len(registered_patients), l4=len(registered_doctors))
...
def approvepatient():
    ...
    try:
        ...
        for i in patient_requests:
            ...
            logger.warning("Пациент для одобрения не найден")
    except Exception as e:
    ...
    registered_patients = ret_registered_patients()
    registered_doctors = ret_registered_doctors()
    return render_template('adminlogin.html', mess='Пациент успешно одобрен',
                           patient_reg_requests=patient_reg_requests, doctor_reg_requests=doctor_reg_requests,
                           registered_patients=registered_patients, registered_doctors=registered_doctors,
                           l1=len(patient_reg_requests), l2=len(doctor_reg_requests), l3=len(registered_patients), l4=len(registered_doctors))
...
def doctorapproveappointment():
    ...
    try:
        ...
        logger.exception("Ошибка при одобрении записи")
...
def deletedoctorrequest():
    ...
    return render_template('adminlogin.html', patient_reg_requests=patient_reg_requests,
                           doctor_reg_requests=doctor_reg_requests, registered_patients=registered_patients,
                           registered_doctors=registered_doctors, l1=len(patient_reg_requests),
                           l2=len(doctor_reg_requests), l3=len(registered_patients), l4=len(registered_doctors))
...
def updatepatient():
    ...
    return render_template('updatepatient.html', fn=fn, ln=ln, dob=dob, phn=phn_db, add=add)
...
def updatedoctor():
    ...
    try:
        ...
        logger.exception("Ошибка при получении данных врача")
        return render_template('updatedoctor.html', mess="Ошибка загрузки данных врача")
...
def makedoctorupdates():
    ...
    try:
        ...
        return render_template('doctorlogin.html', mess='Ошибка при обновлении данных')
...
def makepatientupdates():
    ...
    try:
        ...
        return render_template('patientlogin.html', mess='Ошибка при обновлении данных')
